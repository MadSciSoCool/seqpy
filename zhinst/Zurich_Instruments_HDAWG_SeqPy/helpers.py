...
class SeqcFile:
    ...
    def define_placeholder(self, length, i, j):
        # declare placeholder
        self._writeline(f"wave w_{i:d}_{j:d} = placeholder({length:d}, true);")
        # assign placeholder
...
def readout_seqc_generation(total_length, file_path):
    ...
    for i in range(2):
        seqc_file.define_placeholder(total_length, i, 0)
    seqc_file.start_main_loop(-1)
    seqc_file.awg_monitor_trig()
    seqc_file.play_wave((0, 0, 0), (0, 1, 0))
    seqc_file.end_main_loop()
    return seqc_file
